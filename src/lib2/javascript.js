...
export function generate(
  ...
  for (let sdfX = 0; sdfX < sdfWidth; sdfX++) {
    for (let sdfY = 0; sdfY < sdfHeight; sdfY++) {
      const signedDist = findNearestSignedPseudoDistance(
      ...
      let alpha =
        Math.pow(1 - Math.abs(signedDist) / maxDistance, sdfExponent) / 2;
      if (signedDist < 0) {
        alpha = 1 - alpha;
      }
  ...
  function findNearestSignedDistance(x, y) {
    ...
    for (let i = segments.length; i--; ) {
      ...
      if (
        ...
        const distSq = absSquareDistanceToLineSegment(
          x,
          y,
          seg.x1,
          seg.y1,
  ...
  function findNearestSignedPseudoDistance(x, y) {
    let closestDistSq = Infinity;
    let closestDist = Infinity;
    ...
    for (let i = segments.length; i--; ) {
      ...
      //   y + closestDist > seg.minY
      // ) {
      const distSq = absSquareDistanceToLine(
        x,
        y,
        seg.x1,
        seg.y1,
...
export function generateIntoFramebuffer(
  sdfWidth,
  sdfHeight,
  path,
  viewBox,
...
function absSquareDistanceToLineSegment(x, y, lineX0, lineY0, lineX1, lineY1) {
  ...
  const t = lengthSq
    ? Math.max(
        0,
        Math.min(1, ((x - lineX0) * ldx + (y - lineY0) * ldy) / lengthSq),
      )
    : 0;
...
/**
 * Find the absolute distance from a point to a line at closest approach
 */
function absSquareDistanceToLine(x, y, lineX0, lineY0, lineX1, lineY1) {
  const ldx = lineX1 - lineX0;
  const ldy = lineY1 - lineY0;
  const lengthSq = ldx * ldx + ldy * ldy;
  const t = lengthSq ? ((x - lineX0) * ldx + (y - lineY0) * ldy) / lengthSq : 0;
  const dx = x - (lineX0 + t * ldx);
  const dy = y - (lineY0 + t * ldy);
  return dx * dx + dy * dy;
}
