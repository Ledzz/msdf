...
export function createInstancedText(
  ...
  const layoutPropertiesRef: { current: GlyphLayoutProperties | undefined } = {
    current: undefined,
  };
  ...
  initializers.push(
    ...
    () =>
      effect(() => {
        ...
        if (fonts == null || orderInfo.value == null) {
          return;
        }
...
export class InstancedText {
  ...
  public getCharIndex(x: number, y: number): number {
    ...
    const lineIndex = Math.floor(
      y / -getOffsetToNextLine(layout.lineHeight, layout.fontSize),
    );
    ...
    const whitespaceWidth =
      layout.fonts[" "].getGlyphInfo(" ").xadvance * layout.fontSize;
    ...
    for (let i = 0; i < glyphsLength; i++) {
      ...
      if (
        x <
        this.getGlyphX(entry, 0.5, whitespaceWidth) + layout.availableWidth / 2
      ) {
  ...
  private updateSelectionBoxes(
    ...
    if (endCharIndexExcl <= startCharIndexIncl) {
      const { lineIndex, x } = this.getGlyphLineAndX(
        layout,
        endCharIndexExcl,
        true,
        whitespaceWidth,
    ...
    this.caretPosition.value = undefined;
    const start = this.getGlyphLineAndX(
      layout,
      startCharIndexIncl,
      true,
      whitespaceWidth,
    ...
    const end = this.getGlyphLineAndX(
      layout,
      endCharIndexExcl - 1,
      false,
      whitespaceWidth,
  ...
  private computeSelectionBox(
    ...
    if (endX == null) {
      endX = this.getGlyphX(
        lineGlyphs[lineGlyphs.length - 1],
        1,
        whitespaceWidth,
      );
    }
    ...
    return {
      position: [startX + width / 2, y - height / 2],
      size: [width, height],
    };
  }
  ...
  private getGlyphLineAndX(
    ...
    for (let lineIndex = 0; lineIndex < linesLength; lineIndex++) {
      ...
      //line found
      const glyphEntry =
        this.glyphLines[lineIndex][
          Math.max(charIndex - line.charIndexOffset, 0)
        ];
      return {
        lineIndex,
        x: this.getGlyphX(glyphEntry, start ? 0 : 1, whitespaceWidth),
      };
    }
    ...
    if (lastLine.charLength === 0) {
      return {
        ...
        x:
          getXOffset(availableWidth, lastLine.nonWhitespaceWidth, textAlign) -
          availableWidth / 2,
      };
    }
    const lastGlyphEntry =
      this.glyphLines[linesLength - 1][lastLine.charLength - 1];
    return {
      lineIndex: linesLength - 1,
      x: this.getGlyphX(lastGlyphEntry, 1, whitespaceWidth),
    };
  }

  private getGlyphX(
    entry: number | InstancedGlyph,
  ...
  private show() {
    ...
    this.unsubscribeShowList.push(
      effect(() => {
        ...
        traverseGlyphs(this.glyphLines, (glyph) =>
      ...
      effect(() => {
        ...
        let y =
          getYOffset(layout, this.verticalAlign.value) -
          layout.availableHeight / 2;
        ...
        for (let lineIndex = 0; lineIndex < linesLength; lineIndex++) {
          ...
          const offsetPerWhitespace =
            this.textAlign.value === "block"
              ? (availableWidth - nonWhitespaceWidth) / whitespacesBetween
              : 0;
          let x =
            getXOffset(
              availableWidth,
          ...
          for (
            ...
            if (
              char === " " ||
              charIndex > nonWhitespaceCharLength + firstNonWhitespaceCharIndex
            ) {
              ...
              x +=
                offsetPerWhitespace +
                getOffsetToNextGlyph(fontSize, glyphInfo, letterSpacing);
              continue;
            }
            //non space character
    ...
    case "middle":
      return (
        (layout.availableHeight -
          getGlyphLayoutHeight(layout.lines.length, layout)) /
        2
      );
    case "bottom":
      return (
